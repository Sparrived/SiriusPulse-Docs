...
**宵禁期间最多只留下一条待发的消息**（`intend_share` 会拒绝第二条登记）。原因是天亮后的清空受 `share_cooldown_seconds` 限制：夜里攒下 8 条，就会从 08:00 起每小时放一条，23:00 写的那条可能到次日 15:00 才说出口——"今天的晚霞"到那时早已不是今天的事。上限限的是**积压**而不是"一天一条"：唯一那条发出、队列清空后，新的立刻可以再记。
