...
自主回合内会拒绝一切 `external_write` / `destructive` 类工具，因此她不能在做事的当口顺手往群里发消息；说与做始终是两个决定。唯一豁免的是 `intend_share`（`allowed_when_self_initiated`），因为它只登记、不投递——否则她就没有机会决定"这条没想好受众的话说给谁"。`intend_pursue` **不**豁免：推进一件事时不该顺手登记下一件，否则每个回合都会喂养下一个回合。
