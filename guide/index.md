...
3. [快速开始](./quickstart)：启动 WebUI，创建人格并连接 AMKR。
4. [配置](./configuration)：理解全局配置、人格配置、AMKR 连接和适配器配置。
...
- `data/global_config.json`：全局设置、当前活跃人格与 AMKR 连接信息。
- `data/personas/<name>/`：人格配置、记忆、运行状态和日志相关文件。
- `data/personas/<name>/engine_state/orchestration.json`：人格级编排配置，只保存任务超时与重试等本地传输层参数。
- `data/personas/<name>/tools/`：该人格安装或开发的外部 Tool；内置 Tool 位于核心包的 `sirius_pulse/tools/builtin/`。
- `plugins/`：宿主机共享的外部 Plugin Git submodule，可包含 `github_monitor`、`sub2api_monitor` 等扩展；具体目录以子模块版本为准。`github_monitor` 已外置，不属于核心包或内置 Tool，见 [GitHub Monitor 外部 Plugin](../extensions/github-monitor)。
