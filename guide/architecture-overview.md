...
>
> **纯图片/表情消息处理优化**：v1.5 起，当用户发送的消息仅包含图片或动画表情（无文本内容）时，引擎不再执行完整的认知分析流程（情感、意图、记忆检索等），而是直接调用 `CognitionAnalyzer.describe_image()` 使用多模态 LLM 生成图片描述，并将描述作为 `image_caption` 记录到基本记忆中。此过程仅缓存图片描述以避免重复请求，返回的响应策略固定为 `silent`，仅记录内部状态，不生成回复。这显著降低了纯图片消息处理的 LLM 成本。
>
...
>
> **标签记录（用户消息）**：在处理用户消息时，引擎会提取其中的多模态输入（图片、动画表情等）并生成对应的 `tags` 添加到基本记忆中。例如，动画表情产生 `{ "type": "sticker", "label": "动画表情 ×2" }`，普通图片产生 `{ "type": "image", "label": "图片 ×3" }`。这些标签和图片描述（如非空）会一并记录到基本记忆中，用于后续检索和统计。
>
