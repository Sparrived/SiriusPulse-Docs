...
>
> **意图分数记录**：在认知层分析完成后，引擎会将计算出的意图分数（`social_intent`、`directed_score`、`urgency_score`、`relevance_score`、`sarcasm_score`、`entitlement_score`、`turn_gap_readiness`）附加到刚记录的 `BasicMemoryEntry` 中，并通过 `basic_store.update_entry()` 更新归档文件。这使得每次用户消息的意图分数在重启后仍可查询，并且在 WebUI 对话历史中会显示这些分数标签（如“意图 社交”、“指向 0.75”等）。
>
...
### 6.1 BasicMemoryEntry 字段说明

`BasicMemoryEntry` 是基本记忆的基本单元，用于表示一条用户消息或 AI 回复。主要字段如下：

| 字段 | 类型 | 说明 |
|------|------|------|
| `entry_id` | str | 唯一标识，例如 "human_xxx" 或 "ai_xxx" |
| `group_id` | str | 群组 ID |
| `user_id` | str | 用户 ID（AI 回复时固定为 persona 名称） |
| `role` | str | "human" 或 "ai" |
| `content` | str | 消息文本内容 |
| `timestamp` | str | ISO 格式时间戳 |
| `platform_message_id` | str | 平台消息 ID（用于缓存一致性） |
| `multimodal_inputs` | list[dict] | 多模态输入（图片、表情等） |
| `tags` | list[dict] | 内容标签（表情包、钉住等） |
| `conversation_chain` | list[dict] | 完整 LLM 消息链（已废弃，v1.3 起不再填充） |
| **`intent_scores`** | **dict[str, Any]** | **意图分数（v1.4 新增）**：包含 `social_intent`、`directed_score`、`urgency_score`、`relevance_score`、`sarcasm_score`、`entitlement_score`、`turn_gap_readiness`。默认空字典。该字段由引擎在认知层分析后自动填充，并通过 `BasicMemoryFileStore.update_entry()` 更新到归档文件。WebUI 对话历史页面会显示这些分数标签。|

### 6.2 BasicMemoryFileStore.update_entry()

该方法是 v1.4 新增的，用于通过 `entry_id` 替换归档文件中的已有条目。当需要更新已持久化的 `BasicMemoryEntry`（例如添加 `intent_scores`）时，引擎调用 `store.update_entry(entry)`。如果 entry_id 不存在或文件不可写，返回 False。
...
在对话历史页面，每条用户消息下方会显示一个意图分数区域（`intent_scores` 字段非空时）。该区域会展示：
- 意图类型标签（如"意图 社交"、"意图 情感"）
- 各项分数标签：指向、紧急、相关、讽刺、资格、间隙，保留两位小数（例如"指向 0.75"）

分数标签使用绿色背景样式，与普通标签区分。

### 7.2 合并逻辑优化

在合并来自归档（archive）和运行时（runtime）的消息时，`_merge_conversation_messages` 函数修复了重复 entry_id 的处理：当同一 entry_id 同时存在于归档和运行时中时，会使用 `{**merged[key], **message}` 进行属性合并，确保 `intent_scores`（来自归档）和 `conversation_chain`（来自运行时）都能保留。

---

## 附录：系统提示与输出规范

### A.1 PromptFactory 输出规范（v1.4 更新）

系统提示中强化了 Markdown 格式的禁令。具体说明如下：

> 你的每条回复会被系统按换行符拆分成多条消息发送，所以严禁输出任何Markdown格式（标题#、列表*/-、代码块```、表格|、粗体**、引用>等都会产生大量换行导致刷屏）。如果需要发送格式化内容（如日报、报告），用工具写入文件再通过 send_workspace_file 发送。

同时，在输出规范中明确要求：

- 不要输出 `<message>` XML 标签，不要添加说话者前缀或系统标记。
- **严禁输出Markdown格式**（#标题、*-列表、```代码块、|表格、**粗体**、>引用等）。系统会按换行符拆分消息，Markdown会产生大量空行导致刷屏。
- 需要发送格式化内容时，先用工具写入文件，再调用 `send_workspace_file` 发送。
- 可以通过在开头插入 `[REPLY:msg_id]` 来引用回复某条特定消息。