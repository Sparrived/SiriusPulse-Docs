...
> **新增持久化说明**：在 `BasicMemoryManager.add_entry()` 步骤中，消息会被加入内存窗口（保留所有未检查点的原始消息），并同时通过 `engine.basic_store.append()` 持久化到磁盘。当日记生成器从原始消息中提取记忆单元后，已覆盖的原始消息会从内存窗口中自动修剪。这意味着即使引擎重启，基本记忆仍然可以恢复，对话上下文不会丢失。此持久化同样适用于 AI 回复记录和 SKILL 执行结果。
>
...
flowchart LR
    subgraph BG["内置后台任务（并行运行）"]
        ...
        T3["任务3<br/>日记生成 promoter（含情景提取与补提）<br/>每 180 秒（生成后修剪已覆盖的原始消息）"]
