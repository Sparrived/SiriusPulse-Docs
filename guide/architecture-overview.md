...
flowchart TD
    ...
    subgraph Decision["③ 决策层（纯规则，零 LLM 成本）"]
        ...
        D2 --> D3["ResponseStrategyEngine<br/>IMMEDIATE / DELAYED / SILENT"]
    ...
    subgraph Execution["④ 执行层"]
        ...
        X2 --> X5["PromptFactory.assemble_chat()<br/>组装 prompt"]
        X5 --> X6["StyleAdapter 输出长度/语气指令"]
        X6 --> X7["ModelRouter 选择模型"]
        X7 --> X8["Brain.chat()<br/>→ 获取串行锁<br/>→ 构建 gen_request<br/>→ _call_with_retry(rebuild_fn)<br/>→ Provider.generate_async()"]
        X8 --> X9["解析 function_call (tools)"]
        X9 --> X10["Token 追踪记录"]
        X10 --> X11["emit EXECUTION_COMPLETED"]
    end

    X11 --> U["_background_update()<br/>更新群体氛围 + 群规范学习 + 反馈结算 + 情感孤岛检测"]
```
...
引擎内置 6 个后台任务（v2.0 起移除了主动触发任务和开发者私聊检查任务），另有被动 SKILL 注册的任务（如 reminder）并行运行：

```mermaid
flowchart LR
    subgraph BG["内置后台任务（并行运行）"]
        T1["任务1<br/>延迟队列 ticker<br/>智能休眠（3-30s）"]
        T3["任务2（原任务3）<br/>日记生成 promoter（含情景提取与补提）<br/>每 180 秒"]
        T4["任务3（原任务4）<br/>日记 consolidator<br/>每 600 秒"]
        T6["任务4（原任务6）<br/>表情包新鲜度更新<br/>每 3600 秒"]
        T7["任务5（原任务7）<br/>背景精炼器<br/>每 600 秒"]
        T8["任务6（原任务8）<br/>表情包缓存预热<br/>启动时一次性执行（如无缓存则通过 LLM 生成二元对立关系）"]
    end
