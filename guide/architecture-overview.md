...
| **隐藏计划模式（Plan Mode）** | 允许模型在进行复杂多步推理时进入隐藏模式，中间内容不发送到群聊，完成后一次性输出最终消息；通过 `enter_plan` / `exit_plan` / `abort_plan` 工具控制；`plan_mode_limit_normal_tools` 可在普通聊天中限制工具集，`plan_mode_allow_light_chat` 允许计划进行时关注无关聊天，`plan_mode_presence_enabled` 可在进入计划时发送简略占位消息 |
...
- **隐藏计划模式（Plan Mode）**：v1.5 起新增隐藏计划模式。在编排配置中启用后，模型可调用 `enter_plan` 进入隐藏会话，进行多步工具调用和推理，中间结果不对外发送，直到调用 `exit_plan` 生成最终消息。计划模式支持占位消息（presence）以维持聊天气氛，相关配置项包括 `plan_mode_presence_enabled`、`plan_mode_presence_min_interval_seconds`、`plan_mode_presence_enter_message`、`plan_mode_presence_update_message` 等。占位消息发送受最小间隔控制，避免刷屏。
...
当编排配置中启用 `plan_mode_enabled` 后，引擎支持一种特殊的隐藏回复模式，称为**计划模式**。主要机制如下：
- 模型可通过调用 `enter_plan` 工具进入计划模式，并声明一个具体目标（`goal`）和理由（`reason`）。进入后，后续模型输出（包括中间推理和工具调用结果）**不会发送到群聊**，而是作为私密上下文保留。
- 在计划模式中，工具集切换为只包含 `exit_plan` 和 `abort_plan`，普通技能和流程控制工具被隐藏。
- 计划模式可在同一群聊中与普通聊天共存（`plan_mode_allow_light_chat` 控制）。当普通聊天发生时，引擎会通过 `plan_mode_limit_normal_tools` 限制普通聊天的工具集（仅轻量运行时工具），并自动在系统提示中添加“【计划模式】普通聊天阶段只做轻量可见回复……”的指令。
- 当模型完成任务后，调用 `exit_plan` 并传入 `final_message`（发送到群聊）和可选的 `summary`（私有日志）。也可以调用 `abort_plan` 放弃计划。
- 支持占位消息（presence）：若 `plan_mode_presence_enabled` 为 true，进入计划或收到计划更新事件时，引擎会通过 `_maybe_send_plan_presence` 发送简略文本到群聊（如“我看到了，这个得稍微捋一下。”），受 `plan_mode_presence_min_interval_seconds` 间隔控制，避免刷屏。
- 计划会话状态通过 `plan_runtime` 模块管理，每个群聊仅允许一个活跃计划。计划事件（如外部消息更新）会通过 `format_plan_events_for_model` 格式化为用户消息注入到模型上下文中。
...
sequenceDiagram
    ...
    User->>Engine: “帮我把这3条记录合并成一份报告”
    Note over Engine: 延迟队列触发，lane=chat
    Engine->>Brain: chat(request)
    Brain->>LLM: generate
    LLM-->>Brain: function_call: enter_plan(goal="合并报告")
    Brain-->>Engine: 进入计划模式
    Engine->>Engine: 创建 plan_session，隐藏后续输出
    Note over Engine: 中间工具调用（如 read_record、merge_file）均不发送到群聊
    Engine->>Brain: chat(plan_context)
    Brain->>LLM: generate
    LLM-->>Brain: function_call: exit_plan(final_message="报告已完成！")
    Brain-->>Engine: 结束计划，发送最终消息
    Engine-->>User: “报告已完成！”
```
...
flowchart LR
    ...
    subgraph PassiveSK["被动 SKILL 任务"]
        T9["reminder checker<br/>每 15 秒<br/>通过 create_background_tasks"]
    end
```

> **注意**：计划模式本身不是后台任务，但延迟队列 ticker（任务1）在每次 `tick_delayed_queue` 时负责检查当前群聊是否有活跃计划会话，并注入计划控制工具和 `plan_mode` 相关指令。