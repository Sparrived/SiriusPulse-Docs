...
- 前端页面支持配置自动保存，通过 `autosave.js` 模块实现，在用户输入后延迟自动触发保存，并合并快速连续保存请求，减少重复通知。
...
- **引擎支持配置文件热重载**：通过写入 `engine_state/reload_requested` 标志文件触发，支持类型：`persona`、`orchestration`、`experience`、`provider`、`global`、`all`。其中 `provider` 类型会重新构建 Provider 实例，使 provider 配置变更无需重启引擎。
  - **v1.5 热重载合并与静默期优化**：标志文件改为 JSON 格式 `{"types": ["persona", "provider", ...]}` 以支持合并快速连续保存请求；WebUI 前端自动保存和后端配置保存均会合并类型，避免重复触发。Worker 在检测到文件修改后，会等待至少 2 秒静默期再读取，避免密集写入时重复初始化。解析时支持旧的单字符串格式和新的 JSON 格式。当请求中包含 `all` 时，其他类型被忽略，只执行全部重载。
