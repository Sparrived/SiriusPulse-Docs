...
| 日记促进与精炼 | 可变 | 群聊沉寂后归档对话，在冷寂（COLD）状态下获取 consolidation 候选消息，根据 `memory_idle_consolidation_seconds` 配置判断是否包含上下文；调用 diary_manager.generate_from_candidates 直接生成日记，无需情景提取；生成的切片会持久化到文件并索引到 ChromaDB 向量库 |
