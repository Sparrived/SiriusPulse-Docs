...
此外，`other_mention_score`（消息明确@其他人）的计算规则为：当文本中包含 `@` 符号且 AI 名称或别名**未**在文本中出现时，标记为 `other_mention = 1.0`；若 AI 名称或别名已出现在文本中（例如“月白，@我一下”），则 `other_mention = 0`，避免误将同时提及 AI 和 @ 别人的情况判为 @ 别人。AI 的名称和别名从人格配置中获取，集成到 CognitionAnalyzer 实例中。
