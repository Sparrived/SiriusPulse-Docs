...
engine.memory_unit_manager  # MemoryUnitManager: 结构化记忆单元管理器
```
...
| 记忆检查点 | 可变 | 群聊沉寂后，将 basic memory 条目检查点化为结构化记忆单元；传递 storage 和 user_manager 实例加载群组别名映射及用户信息；从候选消息中过滤已检查点的条目；生成的记忆单元持久化到文件并索引到向量库，成功后标记条目为已检查点 |
| 记忆维护 | 可变 | 语义记忆整理和衰减 |
