...
Brain 为 `chat()` 方法提供了一条默认处理链，按顺序执行以下步骤：

- **pre-hooks**（回复生成前）：人格注入 → 当前时间入消息链 → 模型路由 → 风格覆盖 → 构建请求
- **call**：provider.generate_async()
- **post-hooks**（回复生成后）：XML 剥离 → SKIP 检测 → SKILL 解析 → 表情包解析 → token 记录

> **当前时间**通过 `PromptFactory.build_current_time_section` 生成，并作为用户消息的前缀插入，确保模型感知准确的时间上下文。
...
| 日记促进与精炼 | 可变 | 群聊沉寂后归档对话。结合冷检测、情景提取和演化链精炼；传递 storage 和 user_manager 实例加载群组别名映射及用户信息，提升情景提取准确性；从候选消息中过滤已生成日记或已提取情景的消息；切片会记录关联的情景ID（situation_ids）；在COLD状态下优先使用未处理的情景生成日记，支持情景补提；当候选消息数量超过音量阈值的两倍时，使用话题聚类（TopicCluster）将消息分组，然后为每个聚类调用 `generate_topic_clustered` 方法生成多篇日记，避免信息丢失；否则调用 `generate_from_candidates` 生成单篇日记；生成的切片会持久化到文件并索引到 ChromaDB 向量库，成功后标记情景为已处理 |
