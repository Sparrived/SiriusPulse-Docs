...
| 日记促进与精炼 | 可变 | 群聊沉寂后归档对话，结合冷检测、情景提取和演化链精炼；传递 storage 和 user_manager 实例加载群组别名映射及用户信息，提升情景提取准确性；从候选消息中过滤已生成日记或已提取情景的消息；切片会记录关联的情景ID（situation_ids）；在COLD状态下优先使用未处理的情景生成日记，支持情景补提；生成的切片会持久化到文件并索引到 ChromaDB 向量库，成功后标记情景为已处理，并从基本内存中移除已被覆盖的原始消息 |
...
> **基本内存管理**：BasicMemoryManager 不再使用固定硬上限来裁剪基本内存窗口。后台任务中，当记忆单元成功提升后会自动移除已被覆盖的原始消息，从而在保证上下文完整性的同时避免内存无限增长。`hard_limit` 配置项现仅在恢复旧快照时用于限制最大加载量，默认为 0（不限制）。
