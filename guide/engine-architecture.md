...
> **发送期间消息处理**：当引擎正在向群聊发送多段回复时（通过锁机制保证连续发送），平台层会在新到达消息的 `received_during_bot_send` 标志上标记。引擎在消息到达后（感知阶段前）检查此标志。若为 `True`，则根据消息是否明确点名当前 Bot（通过 `_message_explicitly_mentions_current_bot` 方法检测名称或 @ 提及）决定行为：
> - 点名 Bot：将消息排入延迟回复队列（DELAYED），避免打断当前发送。
> - 未点名：直接返回 SILENT（不回复），不进行认知分析。
> 此机制防止 Bot 在发送多段回复时被无关消息打断或抢话，同时确保点名消息不会被忽略。
