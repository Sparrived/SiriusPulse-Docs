...
非立即回复进入延迟队列，在确认窗口后释放。支持队列合并（连续发送多条消息时合并为一条回复）。在释放回复时，引擎会执行多轮技能调用循环（最多 `max_skill_rounds` 轮）。当所有技能调用均为 `send_sticker` 且没有非技能文本时，引擎会自动进行一轮文本重试：禁用 `send_sticker` 工具并追加提示，要求模型仅输出文本回复，避免仅发送表情包而无文本。
...
ChatRequest 支持 `disabled_skill_names` 字段（`set[str]`），用于在本次调用中禁用特定技能工具（例如在表情包文本重试时禁用 `send_sticker` 技能）。
...
| 任务 | 间隔 | 功能 |
|------|------|------|
| 延迟响应轮询 | 1s | 释放到期延迟回复（含技能调用循环及表情包文本重试） |
