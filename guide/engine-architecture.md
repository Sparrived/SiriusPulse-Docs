...
当延迟回复被释放并生成回复时，引擎会进入一个多轮交互循环。AI 可以通过调用内置的**流程控制工具**来决定后续行为：

- **continue**：发送当前文字并继续生成下一条消息。调用后，已输出的文字会发送给用户，然后 AI 可以继续生成新的内容。
- **stop**：结束本轮回复。最后一条文字消息会发送给用户，然后本轮回复结束。
...
CONTINUE_TOOL_DEF = {
    ...
    "function": {
        "name": "continue",
        "description": "发送当前回复并继续生成下一条。调用后，已输出的文字会发送给用户，然后你可以继续生成新的内容。如果你想在同一轮中发送多条消息，就在每条消息后调用 continue。",
        "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": ["continue"]}}, "required": ["action"]},
    },
}
...
STOP_TOOL_DEF = {
    ...
    "function": {
        ...
        "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": ["stop"]}}, "required": ["action"]},
    },
}
```

在每次回复生成时，这两个工具会附加到可用的工具列表中。如果 AI 未调用任何工具，则隐式视为 stop，当前文字作为最终回复。如果调用 continue，引擎会立即发送当前文字（排除所有 silent 技能的输出），然后继续下一轮生成。如果调用 stop，则发送完当前文字后结束循环。

#### 技能执行与流控协作

在多轮交互中，AI 可以同时调用普通技能和流程控制工具。普通技能的执行结果会以 tool 消息的形式注入下一轮对话。引擎会追踪本轮开始时的最近 5 条消息内容，避免因 continue 导致消息重复注入。当 continue 后，新的用户消息会被注入到消息链中（实际上是从 basic_memory 获取最新消息），使 AI 能够感知群聊的变化。
...
| 任务 | 间隔 | 功能 |
|------|------|------|
| 延迟响应轮询 | 1s | 释放到期延迟回复，支持流程控制工具（continue/stop）实现多轮交互 |
