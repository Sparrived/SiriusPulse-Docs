...
> **自动热重载**：更新 Provider 配置后，系统会自动向所有运行中的人格发送 `provider` 热重载信号，无需手动重启人格即可使新配置生效。若连续多次保存配置，系统会合并重载类型，在最后保存后等待 2 秒静默期再执行一次性热重载，避免重复初始化。若通过 `POST /api/providers/refresh-models` 刷新模型列表，也会触发同样的热重载。
