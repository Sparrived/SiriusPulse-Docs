...
AMKR 相关的四个字段详见 [AMKR 接入配置参考](./provider-config)；`amkr_local_api_key` 是 AMKR 的管理员凭据，WebUI API 只回显掩码。
...
| 变量 | 用途 |
|---|---|
| `SIRIUS_AMKR_BASE_URL`、`SIRIUS_AMKR_API_KEY`、`SIRIUS_AMKR_WORKSPACE` | 覆盖 `global_config.json` 中的 AMKR 连接配置，环境变量优先。 |
